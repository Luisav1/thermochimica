...
program TestMQMQASUBQGEMMappingVerification
    ...
    real(8) :: dConstraintMuCorrected, dConstraintMuBaseline, dDeltaBMagnitude, dFloorDifference
    ...
    ! toward the uniform composition to obtain a controlled positive interior
    ! state while retaining the decoded production topology and parameters.
    dX = 0.8D0*dX+0.2D0/DFLOAT(nQuad)
    do e = 1, nElements
    ...
    allocate(dHessian(nQuad,nQuad),dHx(nQuad,nQuad),dHbase(nQuad,nQuad), &
        ...
        dMuShiftResponseBase(nQuad,1),dLambdaElement(1,nElements),dLambdaElementBase(1,nElements), &
        dLambdaMu(1,1),dLambdaMuBase(1,1),dLambdaMuShift(1,1),dLambdaMuShiftBase(1,1), &
        dDeltaA(nElements,nElements),dDeltaB(nElements),dACandidate(nElements,nElements), &
        dBCandidate(nElements),dZ(nQuad,nQuad-1))
    ...
    call SolveConstrainedResponse(dHx,dConstraint,dForcing,dResponse,iInfo,dLambdaElement)
    lPass = lPass .AND. (iInfo == 0)
    call SolveConstrainedResponse(dHbase,dConstraint,dForcing,dResponseBase,iInfo,dLambdaElementBase)
    lPass = lPass .AND. (iInfo == 0)
    dForceMu(:,1) = dMu-1D0
    dForceMuShifted(:,1) = dMu
    call SolveConstrainedResponse(dHx,dConstraint,dForceMu,dMuResponse,iInfo,dLambdaMu)
    lPass = lPass .AND. (iInfo == 0)
    call SolveConstrainedResponse(dHbase,dConstraint,dForceMu,dMuResponseBase,iInfo,dLambdaMuBase)
    lPass = lPass .AND. (iInfo == 0)
    call SolveConstrainedResponse(dHx,dConstraint,dForceMuShifted,dMuShiftResponse,iInfo,dLambdaMuShift)
    lPass = lPass .AND. (iInfo == 0)
    call SolveConstrainedResponse(dHbase,dConstraint,dForceMuShifted,dMuShiftResponseBase,iInfo, &
        dLambdaMuShiftBase)
    lPass = lPass .AND. (iInfo == 0)
    ...
    dTopElementBaseline = KKTTopError(dHbase,dConstraint,dResponseBase,dLambdaElementBase,dForcing)
    dTopMuCorrected = KKTTopError(dHx,dConstraint,dMuResponse,dLambdaMu,dForceMu)
    dTopMuBaseline = KKTTopError(dHbase,dConstraint,dMuResponseBase,dLambdaMuBase,dForceMu)
    dConstraintElementCorrected = MAXVAL(DABS(MATMUL(dConstraint,dResponse)))
    dConstraintElementBaseline = MAXVAL(DABS(MATMUL(dConstraint,dResponseBase)))
    dConstraintMuCorrected = MAXVAL(DABS(MATMUL(dConstraint,dMuResponse)))
    dConstraintMuBaseline = MAXVAL(DABS(MATMUL(dConstraint,dMuResponseBase)))
    dConstraintResidual = MAX(dConstraintElementCorrected,dConstraintElementBaseline, &
        dConstraintMuCorrected,dConstraintMuBaseline)
    dConstantForceError = VectorError(dMuResponse(:,1),dMuShiftResponse(:,1))
    dBaselineConstantForceError = VectorError(dMuResponseBase(:,1),dMuShiftResponseBase(:,1))
    ...
    lPass = lPass .AND. (MAXVAL(DABS(dACandidate)) == 0D0) .AND. &
        ...
        (dTopElementCorrected <= 1D-10) .AND. (dTopElementBaseline <= 1D-10) .AND. &
        (dTopMuCorrected <= 1D-10) .AND. (dTopMuBaseline <= 1D-10) .AND. &
    ...
    if (lReport) then
        ...
        write(*,'(A,ES14.6)') 'corrected mu versus mu-1 response difference = ',dConstantForceError
        write(*,'(A,ES14.6)') 'baseline mu versus mu-1 response difference = ',dBaselineConstantForceError
        write(*,'(A,ES14.6)') 'corrected element-response KKT top residual = ',dTopElementCorrected
        write(*,'(A,ES14.6)') 'baseline element-response KKT top residual = ',dTopElementBaseline
        write(*,'(A,ES14.6)') 'corrected mu-1-response KKT top residual = ',dTopMuCorrected
        write(*,'(A,ES14.6)') 'baseline mu-1-response KKT top residual = ',dTopMuBaseline
        write(*,'(A,ES14.6)') 'corrected element-response constraint residual = ', &
            dConstraintElementCorrected
        write(*,'(A,ES14.6)') 'baseline element-response constraint residual = ',dConstraintElementBaseline
        write(*,'(A,ES14.6)') 'corrected mu-1-response constraint residual = ',dConstraintMuCorrected
        write(*,'(A,ES14.6)') 'baseline mu-1-response constraint residual = ',dConstraintMuBaseline
        ...
        if (iBestA > 0) write(*,'(A,ES14.6)') &
            'best resolved mixed-forcing deltaA*dGamma maximum scaled component error = ', &
            dAComponents(iBestA)
...
contains
    ...
    !> Obtain the complete production MQMQA partial molar without double counting.
    !!
    !! The low-level routine retains its historical SUBG name, but dispatches
    !! internally according to the active SUBG or SUBQ model branch.
    ...
    subroutine EvaluateDeltaResponse(dXBase,dForce,dAmplitude,dCorrPlus,dCorrMinus,dIdealPlus,dIdealMinus, &
        ...
        real(8), allocatable :: dResidualPlus(:), dResidualMinus(:)
        real(8), allocatable :: dCorrectionMinus(:,:), dCorrectionPlus(:,:), dMappedCorrection(:,:)
        real(8), allocatable :: dJacobianMinus(:,:), dJacobianPlus(:,:)
        integer :: iInfoLocal
        allocate(dResidualPlus(SIZE(dXBase)),dResidualMinus(SIZE(dXBase)), &
            dCorrectionMinus(SIZE(dZ,2),1),dCorrectionPlus(SIZE(dZ,2),1), &
            dMappedCorrection(nElements,2),dJacobianMinus(SIZE(dZ,2),SIZE(dZ,2)), &
            dJacobianPlus(SIZE(dZ,2),SIZE(dZ,2)))
        ...
        ! Each root's remaining stationarity residual is translated with that
        ! root's production-FD tangent Jacobian. The analytic SUBQ Hessian being
        ! tested does not participate in the oracle-resolution classification.
        dCorrectionPlus(:,1) = MATMUL(TRANSPOSE(dZ),dResidualPlus)
        dCorrectionMinus(:,1) = MATMUL(TRANSPOSE(dZ),dResidualMinus)
        call SolveDense(dJacobianPlus,dCorrectionPlus,iInfoLocal)
        if (iInfoLocal == 0) call SolveDense(dJacobianMinus,dCorrectionMinus,iInfoLocal)
        if (iInfoLocal /= 0) then
            ...
            dMappedCorrection(:,1) = dN*MATMUL(TRANSPOSE(dS),MATMUL(dZ,dCorrectionPlus(:,1)))
            dMappedCorrection(:,2) = dN*MATMUL(TRANSPOSE(dS),MATMUL(dZ,dCorrectionMinus(:,1)))
            dMappedUncertainty = (DABS(dMappedCorrection(:,1))+DABS(dMappedCorrection(:,2)))/ &
                (2D0*DABS(dAmplitude))
        end if
        deallocate(dResidualPlus,dResidualMinus,dCorrectionMinus,dCorrectionPlus, &
            dMappedCorrection,dJacobianMinus,dJacobianPlus)
    ...
    subroutine SolveProductionForcedState(dXBase,dForce,dAmplitude,dXSolved,dResidual,lSolved, &
        ...
        if (PRESENT(dTangentJacobian)) then
            if ((SIZE(dTangentJacobian,1) /= nQuad-1) .OR. &
                (SIZE(dTangentJacobian,2) /= nQuad-1)) then
                lSolved = .FALSE.
            else if (iInfoLocal == 0) then
                call BuildProductionTangentJacobian(dXSolved,dTangentJacobian,iInfoLocal)
                lSolved = lSolved .AND. (iInfoLocal == 0)
            end if
        end if
    ...
    !> Build the oracle tangent Jacobian from finite differences of production partial molars.
    subroutine BuildProductionTangentJacobian(dXLocal,dJacobianLocal,iInfoLocal)
        real(8), intent(in) :: dXLocal(:)
        real(8), intent(out) :: dJacobianLocal(:,:)
        integer, intent(out) :: iInfoLocal
        integer :: iDirection
        real(8) :: dDifference
        real(8), allocatable :: dDistance(:), dMuMinus(:), dMuPlus(:)

        allocate(dDistance(nQuad),dMuMinus(nQuad),dMuPlus(nQuad))
        dJacobianLocal = 0D0
        iInfoLocal = 0
        do iDirection = 1, nQuad-1
            where (DABS(dZ(:,iDirection)) > 0D0)
                dDistance = dXLocal/DABS(dZ(:,iDirection))
            elsewhere
                dDistance = HUGE(1D0)
            end where
            dDifference = DMIN1(1D-5,0.1D0*MINVAL(dDistance))
            call EvaluateProductionMu(iPhaseIndex,iFirst,iLast, &
                dXLocal-dDifference*dZ(:,iDirection),dMuMinus,iInfoLocal)
            if (iInfoLocal /= 0) exit
            call EvaluateProductionMu(iPhaseIndex,iFirst,iLast, &
                dXLocal+dDifference*dZ(:,iDirection),dMuPlus,iInfoLocal)
            if (iInfoLocal /= 0) exit
            dJacobianLocal(:,iDirection) = MATMUL(TRANSPOSE(dZ),dMuPlus-dMuMinus)/ &
                (2D0*dDifference)
        end do
        deallocate(dDistance,dMuMinus,dMuPlus)
    ...
    !> Evaluate the complete upper KKT equation using the multiplier returned by the constrained solve.
    real(8) function KKTTopError(dCurvature,dConstraintLocal,dResponseLocal,dMultiplierLocal,dForcingLocal)
        real(8), intent(in) :: dCurvature(:,:), dConstraintLocal(:,:), dResponseLocal(:,:)
        real(8), intent(in) :: dMultiplierLocal(:,:), dForcingLocal(:,:)
        real(8), allocatable :: dResidual(:,:)

        allocate(dResidual(SIZE(dForcingLocal,1),SIZE(dForcingLocal,2)))
        dResidual = MATMUL(dCurvature,dResponseLocal) + &
            MATMUL(TRANSPOSE(dConstraintLocal),dMultiplierLocal) - dForcingLocal
        KKTTopError = MAXVAL(DABS(dResidual))/DMAX1(1D0,MAXVAL(DABS(dForcingLocal)))
        deallocate(dResidual)
    end function KKTTopError
    ...
    subroutine FinishTest(lAllPass)
        ...
        call ResetThermoAll
        if (lAllPass) then
            print *, 'TestMQMQASUBQGEMMappingVerification: PASS'
            call EXIT(0)
        else
            print *, 'TestMQMQASUBQGEMMappingVerification: FAIL <---'
            call EXIT(1)
        end if
    end subroutine FinishTest

end program TestMQMQASUBQGEMMappingVerification
